...
# Modified, originally from https://github.com/blueimp/shell-scripts/blob/master/bin/wait-for.sh
# Modification: instead of directly invoking 'nc', invoke it via docker (since the target is in docket network and not exposed to host)
...
connect_to_service() {
  docker run --rm --network cbam_test_nw alpine nc -w 1 -z "$1" "$2"
}
