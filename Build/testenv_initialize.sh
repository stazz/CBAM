...
docker create --rm --expose 4222 --name cbam_test_nats --cidfile "${STATE_DIR}/cid_nats" nats:1.3.0-linux
docker create --rm --expose 5432 --name cbam_test_pgsql -e POSTGRES_PASSWORD=postgres --cidfile "${STATE_DIR}/cid_pgsql" postgres:11.1-alpine
...
"${SCRIPTDIR}/wait-for.sh" -t 60 cbam_test_nats:4222 cbam_test_pgsql:5432
"${SCRIPTDIR}/wait-for.sh" -t 60 cbam_test_pgsql_ssl:5432 || docker logs cbam_test_pgsql_ssl